...
import { CalendarDays, ClipboardCheck, Film } from "lucide-react";
...
export default function ProgressTrail() {
  ...
  if (loading) {
    return (
      <Card>
        ...
        <CardContent>
          <div className="no-scrollbar -mx-4 flex w-[calc(100%+2rem)] overflow-x-auto px-4 md:mx-0 md:w-auto md:overflow-visible">
            <div className="flex min-w-max items-center gap-4 md:min-w-0 md:flex-wrap md:justify-center">
              {Array(8).fill(0).map((_, index) => (
                <div key={index} className="flex min-w-[92px] flex-col items-center gap-2 px-2">
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle className="text-center font-headline text-xl font-bold md:text-left">
          Seu Progresso na Trilha
        </CardTitle>
      </CardHeader>

      <CardContent>
        {/* Faixa rolável no mobile; wrap no desktop */}
        <div
          className="
                    ...
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="group flex w-24 shrink-0 flex-col items-center gap-2 text-center">
                          ...
                          <p className="px-1 text-xs font-semibold leading-tight text-muted-foreground transition-colors group-hover:text-primary">
                            {item.title}
                          </p>
                        </div>
                      </TooltipTrigger>
